...
class ProfileController extends FrontendController
{
    ...
    public function store(Request $request){
        ...
        $request->validate([
            ...
            'phone.phone' => 'Contact number must be a valid contact number.',
        ...
        if(!empty($languages = array_filter($request->input('languages') ?? [])))
        {
        ...
        if(!empty($skills = array_filter($request->skills ?? []))){
        ...
        if(!empty($categories = array_filter($request->categories ?? []))){
